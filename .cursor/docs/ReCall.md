...
- **Active branch:** `MSC-Projectz-FullDev-v6` (always confirm with `git branch --show-current`). Prior lines: **`MSC-Projectz-FullDev-v5`**, **`v4`**, **`v3`** on `origin` for history.
...
- **Calendar:** **`/calendar`** — Phase **9** responsive grid, day-detail **Dialog**, **`CalendarGrid`** / **`MSC-Projectz-Calendar`**; docs in **`Development-Roadmap.md`**, **`Session-Snapshots`**, **`Project-Truth`** / **`START-HERE`**. **`useIsMaxMd`** where sub-`**md`** sheet/agenda applies. **Primary branch:** **`MSC-Projectz-FullDev-v6`**.
...
- **`START-HERE.md`:** the canonical primary line is **`MSC-Projectz-FullDev-v6`** — see *Current Restart Point* for the live SHA (prior checkpoints **v5** / **v4** / **v3** on **`origin`**).  
